...
$mark2_price_lists_gen_config = array(
	...
	array(
		'type'=>'3',
		'title'=>'yandex',
		'form_title'=>'Yandex Market YML Цена за штуку',
		'file_name'=>'yandex2.yml',
		'header'=>'<?xml version="1.0" encoding="UTF-8"?><yml_catalog date="'.date('Y-m-d H:i:s').'"><shop>',
		'footer'=> '</shop></yml_catalog>',
		'site_url'=>'https://napitki.ru',
		'make_categories'=>1,
		...
		'conf' => array(
				...
				array(//12
					...
					'field_to_get'=>'price_value',
					'type_field'=>'type',
					'type_id'=>'10',  //Старая цена за штуку
					'evl'=>3,
					'title'=>'Старая цена за упаковку',
					'template'=>'<oldprice>{__val__}</oldprice>',
				),
				...
				array(//0
					...
					'field_to_get'=>'type_value_str',
					'type_field'=>'type_id',
					'type_id'=>'141',  //Штук в упаковке 
					'title'=>'Салес нотес',//фиксировано из реестра со вставкой значения количества
					'evl'=>11,
					'template'=>'<sales_notes>{__val__}</sales_notes>',
				),
				array(//0
	...
	array(
		'type'=>'4',
		'title'=>'google',
		'form_title'=>'Google Merchant XML RSS2.0 Цена за штуку',
		'file_name'=>'products2.xml',
		'header'=>'<?xml version="1.0"?><rss version="2.0" xmlns:g="http://base.google.com/ns/1.0"><channel>',
		'footer'=> '</channel></rss>',
		'site_url'=>'https://napitki.ru',
		'titles'=>'
...
//		return substr($val,0,-4).'.png';
	...
	if($in == 11)//sales notes 2
	{
		$sn = 0;
		$sn = glob::get('sales_notes_2');
		if(!$sn)
		{
		  $sn = registry::get('sales_notes_2');
		}
		$r = str_replace('val',$val,$sn);
		return $r;
	}
